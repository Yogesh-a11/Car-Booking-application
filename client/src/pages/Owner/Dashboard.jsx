...
const Dashboard = () => {
  ...
  return (
    <div className='px-4 pt-10 md:px-10 flex-1'>
      <Title title="Admin Dashboard" subTitle="Monitor overall platform performance including total cars, revenue and recent activities"/>
    ...
    <div className='flex flex-wrap items-start gap-6 mb-8 w-full'>
      ...
      <div className='p-4 md:p-6 border border-borderColor rounded-md max-w-lg w-full'>
        ...
        {data.recentBookings.map((booking, index) => (
          <div key={index} className='flex justify-between mt-4'>
            <div className='flex items-center gap-2'>
              <div className='hidden md:flex items-center justify-center w-12 h-12 bg-primary/10'>
                <img src={assets.listIconColored} alt="" className='w-5 h-5'/>
              </div>
              <div>
                <p>{booking.car.brand} {booking.car.model}</p>
                <p className='text-gray-500 text-sm'>{booking.createdAt.split("T")[0]}</p>
              </div>
            </div>
        ...
        {/* monthly revenue */}
      <div className='p-4 md:p-6 border mb-6 border-borderColor rounded-md w-full md:max-w-xs'>
        <h1 className='font-medium text-lg'>Monthly Revenue</h1>
        <p className='text-gray-500'>Revenu for current month</p>
        <p className='font-semibold text-primary mt-6 text-3xl'>{currency}{data.monthlyRevenue}</p>
      </div>
